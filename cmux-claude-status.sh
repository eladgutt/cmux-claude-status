...
render() {
    ...
    case "$state" in
        running)    icon=bolt.fill;             color='#E8833A'; text="$(fmt_clock "$epoch") crunching";;
...
case "$EVENT" in
    ...
    busy)
        ...
        # needsInput/idle row left over from before this turn. A no-op if
        # already running: the timestamp doesn't move once set.
        ...
        rm -f "$STATE_FILE" "$BG_MARKER" "$MODE_FILE" "$DIR/${CMUX_SURFACE_ID:-$CMUX_TAB_ID}.info-stamp"
        ;;
    tick)
