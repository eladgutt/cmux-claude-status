...
- **Routing badge** - a teal `V` badge for sessions routed through a custom `ANTHROPIC_BASE_URL` containing `valar`, a yellow `A` badge for sessions going direct to Anthropic. Teal/yellow deliberately: the obvious brand colors (gateway green, Anthropic clay-orange) sit on top of the idle/crunching status colors, and routing must never be confusable with state at a glance. Adapt the one-line match in `valar_row()` to badge whatever gateway you use, or make the direct branch a `clear_meta` if you don't want the `A` row.
